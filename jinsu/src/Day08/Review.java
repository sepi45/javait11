...
class Test2 extends Test{		//Test에 있는 변수와 메서드가 복붙됨
	...
	@Override
	public void setVar(int num, String str) {
		super.setVar(num, str);		//Test클래스의 setVar를 사용
		// 수정 == 오버라이딩 ==재정의 : 우선순위가 자신것이 더 높음
		this.dNum = 0.0;
		}	//오버라이딩 : 상속받은 후 재정의
	...
	public void setVar(int num, String str, double dNum) {
		...
		this.dNum =dNum;
	}		//오버로딩 : 메서드 이름 같아도 매개변수가 다르면 다른 메서드로 취급(같은 이름의 메서드 여러개)
	
	//추가
	public double getdNum() {
		super.getstr();
		this.getstr();		//수정 =>우선순위는 this가 더 높음
		return dNum;
	}
}